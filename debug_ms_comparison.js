...
async function debugMassStart() {
    ...
    const menOfficial = parseMSSheet('Men - MS');
    const menApp = state.soqc['Mass Start-men'].quotas.qualified.concat(state.soqc['Mass Start-men'].quotas.reserve);

    const fs = require('fs');
    let output = '--- Men Mass Start Comparison ---\n';
    output += 'Rank | Official Name (Pts) | App Name (Pts)\n';

    const maxLen = Math.max(menOfficial.length, menApp.length);
    for (let i = 0; i < 40; i++) { // Top 40
        const off = menOfficial[i] || {};
        const app = menApp[i] || {};
    ...
    fs.writeFileSync('debug_ms_output.txt', output);
    console.log('Output written to debug_ms_output.txt');
}
